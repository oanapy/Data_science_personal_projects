...
search_data_train <- read.csv("train_sample.csv")
search_data_test <- read.csv("test_sample.csv")
...
train_sub_exercise$random_variable <- runif(93685, 0, 1000)
test_sub_exercise$random_variable <- runif(21449, 0, 1000)

# train_sub_exercise$random_variable <- 
# test_sub_exercise$random_variable <- 

# 3. retrain the same model
fit_rfTree_exercise

search_dat_train_exercise['y_hat'] <- predict(fit_rfTree, data = train_sub_exercise)$predictions[, 2]
search_dat_test_exercise['y_hat'] <- predict(fit_rfTree, data = test_sub_exercise)$predictions[, 2]
...
#  you can use function mispredicted_hotels and set correctly_predicted="no"

mispredicted_hotels(search_data_train, correctly_predicted = 'no')


# 2. create a subset dataframe that contains only this search
# these are the two seearches I have used, you can chose others
...
# you can use function the_recommended_one and pass the subset one_search_exercise dataframe 
...
# you can use function the_booked_one and pass the subset one_search_exercise dataframe 
...
# if you have followed the aboe structure you will have the mispredicted hotel in dataframe one_pred_exercise
lime_pred_exercise <- 
# you can use the below code to plot them next to each other
...
# if you have followed the aboe structure you will have the selected hotel in dataframe one_book_exercise
lime_book_exercise <- 
# you can use the below code to plot them next to each other
par(mfrow=c(1,2))
